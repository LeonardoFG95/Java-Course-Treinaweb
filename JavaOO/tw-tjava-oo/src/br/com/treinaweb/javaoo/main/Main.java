...
//			((Carro)corsa).setQuatidadePortas(4);
			...
			corsa.ligar();
			corsa.acelerar();
			System.out.println(String.format("Velocidade atual no veículo: %f", corsa.getVelocidade()));
			corsa.frear();
			System.out.println(String.format("Velocidade atual no veículo: %f", corsa.getVelocidade()));
		} catch (AbastecimentoVeiculoLigadoException e) {
			System.out.println("Você não pode abastecer um veículo enquanto elke estiver ligado.");
